...
def convert_audio():
    ...
    try:
        ...
        def cleanup():
            paths_to_clean = [input_path, output_path]
...
def merge_audio():
    ...
    try:
        ...
        @response.call_on_close
        def cleanup():
            all_paths_to_remove = file_paths + [output_path]
            try:
                for path in all_paths_to_remove:
                    if path:
                        os.remove(path)
            except:
                pass
        ...
        status_code = 400 if 'Invalid file' in str(e) or 'At least 2 files' in str(e) or 'Missing output_format' in str(e) else 500
...
def export_audio():
    ...
    try:
        ...
        if not output_format or not bitrate or not sample_rate_str:
            return jsonify({'error': 'Missing required export settings (format, bitrate, or sample_rate)'}), 400
        ...
        status_code = 400 if 'Sample rate must be' in str(e) or 'Missing required' in str(e) else 500
        return jsonify({'error': str(e)}), status_code
