...
const VerticalSlider = ({ value, onChange, min, max }) => {
  ...
  return (
    <div className={`${styles.verticalSliderContainer} ${isDragging ? styles.dragging : ""}`} onMouseDown={handleMouseDown}>
      <div className={styles.verticalSliderTrack} ref={trackRef}>
        <div className={styles.centerLine} />
        <div className={`${styles.verticalSliderFill} ${isChanged ? styles.changed : styles.default}`} style={{ bottom: value >= 0 ? "50%" : `${percentage}%`, height: `${Math.abs(value)/(max-min)*100}%` }} />
        <div className={`${styles.verticalSliderKnob} ${isDragging ? styles.knobDragging : ""}`} style={{ bottom: `calc(${percentage}% - 6px)` }} />
      </div>
    </div>
  );
};
...
const FrequencyResponseCurve = ({ bandValues }) => {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className={styles.frequencyResponseSvg} preserveAspectRatio="none">
      <defs>
      ...
      {[-12,-6,0,6,12].map(db=>{
        ...
        return <line key={db} x1={padding.left} y1={y} x2={width-padding.right} y2={y} stroke={db===0?"#444":"#2a2a2a"} strokeWidth={db===0?1:0.5} />;
      })}

      <path d={generateFillPath()} fill="url(#curveGradient)" />
      <path d={generateCurvePoints()} fill="none" stroke="url(#lineGradient)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />

      {bandValues.map((val,i)=>{
        const x=padding.left+(i/(bandValues.length-1))*graphWidth;
        const y=padding.top+graphHeight/2-(val/12)*(graphHeight/2);
        return <circle key={i} cx={x} cy={y} r="3" fill="#17e1ff" stroke="#0f0f0f" strokeWidth="1" />;
      })}
    </svg>
  );
};
...
const Equalizer = ({ isOpen, onClose, initialPosition={x:100,y:100} }) => {
  ...
  const handleBandChange = useCallback(async (index, value) => {
    ...
    if (selectedTrackId) {
      ...
      try {
        localStorage.setItem('webamp.masterEffects', JSON.stringify(newMasterEffects));
      } catch (e) {
  ...
  const handlePresetChange = useCallback(async (presetKey) => {
  ...
  return (
    <div ref={panelRef} className={`${styles.equalizerPanel} ${isDragging ? styles.dragging : ""}`} style={{ left: position.x, top: position.y }} onMouseDown={handleMouseDown}>
