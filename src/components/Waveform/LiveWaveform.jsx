...
export default function LiveWaveform({ stream, startTs, pixelsPerSecond = 120 }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const elapsedSec = Math.max(0, (Date.now() - (startTs || Date.now())) / 1000);
      const x = Math.min(canvas.width - 1, Math.floor(elapsedSec * pixelsPerSecond));
    ...
    return () => {
      ...
      try { nodesRef.current.source?.disconnect?.(); } catch {}
      try { nodesRef.current.analyser?.disconnect?.(); } catch {}
      try { nodesRef.current.audioCtx?.close?.(); } catch {}
