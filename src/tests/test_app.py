...
from backend.app import app, allowed_file, ALLOWED_EXTENSIONS 
...
def mock_send_file():
    """
    FIX: Fixture now yields the MOCK FUNCTION (mock_func) itself, 
    allowing tests to assert call history, which fixes the 'Called 0 times' errors.
    """
...
def test_convert_audio_success(mock_remove, client, mock_processor, mock_send_file):
    """Tests successful audio conversion via the /convert route."""
    mock_processor.convert_format.return_value = '/tmp/output.mp3'
    data = {
    ...
    mock_send_file.return_value.call_on_close.assert_called_once()
    cleanup_func = mock_send_file.return_value.call_on_close.call_args[0][0]
    cleanup_func()
    assert mock_remove.call_count == 2

@patch('backend.app.os.remove')
def test_convert_audio_missing_input(mock_remove, client, mock_processor):
    """Tests /convert when the required 'file' or 'target_format' is missing."""
    
    # Case 1: Missing file
    response_no_file = client.post('/convert', data={'target_format': 'mp3'}, content_type='multipart/form-data')
    ...
    response_no_format = client.post('/convert', data=data_no_format, content_type='multipart/form-data')
    assert response_no_format.status_code == 400
    assert response_no_format.json['error'] == 'Missing target_format parameter'
    
    mock_processor.convert_format.assert_not_called()
    
@patch('backend.app.os.remove')
def test_convert_audio_invalid_file_type(mock_remove, client, mock_processor):
...
@patch('backend.app.os.remove')
def test_convert_audio_processor_failure(mock_remove, client, mock_processor):
    """Tests /convert when the AudioProcessor raises an exception."""
    mock_processor.convert_format.side_effect = Exception("Format validation failed")
...
@patch('backend.app.os.remove')
@patch('backend.app.uuid.uuid4', side_effect=[MagicMock(hex='f1_id'), MagicMock(hex='f2_id')])
def test_merge_audio_success(mock_uuid, mock_remove, client, mock_processor, mock_send_file):
    """Tests successful audio merging via the /merge route."""
    mock_processor.merge_files.return_value = '/tmp/merged.ogg'
    data = {
        'files[]': [
    ...
    assert response.status_code == 200
    mock_processor.merge_files.assert_called_once()
    mock_send_file.assert_called_once_with(
        '/tmp/merged.ogg',
        as_attachment=True,
...
@patch('backend.app.os.remove')
@patch('backend.app.uuid.uuid4', side_effect=[MagicMock(hex='f1_id'), MagicMock(hex='f2_id')])
def test_merge_audio_one_invalid_file(mock_uuid, mock_remove, client, mock_processor):
    ...
    mock_processor.merge_files.assert_not_called()

def test_merge_audio_error_too_few_files(client, mock_processor):
...
@patch('backend.app.os.remove')
def test_export_audio_success(mock_remove, client, mock_processor, mock_send_file):
    """Tests successful audio export with specific settings via /export route."""
    mock_processor.export_with_settings.return_value = '/tmp/exported.flac'
    data = {
    ...
    mock_processor.export_with_settings.assert_called_once_with(
        mock_processor.export_with_settings.call_args[0][0],
        'flac',
        '1411k',
        96000
    )
...
@patch('backend.app.os.remove')
def test_export_audio_missing_required_params(mock_remove, client, mock_processor):
    """Tests /export when mandatory parameters like format, bitrate, or sample_rate are missing."""
    ...
    assert response.status_code == 400
    assert 'Missing required export settings (format, bitrate, or sample_rate)' in response.json['error']
    mock_processor.export_with_settings.assert_not_called()

@patch('backend.app.os.remove')
def test_export_audio_invalid_sample_rate(mock_remove, client, mock_processor):
    """Tests /export when sample_rate is provided but is not a valid integer."""
...
def test_get_metadata_success(mock_uuid, mock_remove, client, mock_processor):
    ...
    mock_processor.get_audio_info.return_value = expected_metadata
    data = {
        'file': create_mock_file('info.mp3', 'audio/mp3')
    }
    response = client.post('/metadata', data=data, content_type='multipart/form-data')
    assert response.status_code == 200
    assert response.json == expected_metadata

    mock_processor.get_audio_info.assert_called_once()
    assert mock_remove.call_count == 1
    assert 'meta_id_info.mp3' in mock_remove.call_args[0][0]

@patch('backend.app.os.remove')
def test_get_metadata_invalid_file_type(mock_remove, client, mock_processor):
    ...
    assert response.status_code == 400
    assert response.json['error'] == 'Invalid file type'
    mock_processor.get_audio_info.assert_not_called()

@patch('backend.app.os.remove')
def test_get_metadata_no_file(mock_remove, client, mock_processor):
    """Tests metadata endpoint with no file provided."""
    response = client.post('/metadata', data={}, content_type='multipart/form-data')
