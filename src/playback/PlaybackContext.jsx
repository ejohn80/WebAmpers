...
export function PlaybackProvider({version, children}) {
  ...
  useEffect(() => {
    ...
    engine
      ...
      .then(() => {
        ...
        try {
          engine.master && (engine.master.gain.gain.value = 1);
        } catch {}
        setMasterVol(100);
      })
      .catch((e) =>
        console.error("[PlaybackProvider] engine.load() failed:", e)
      );
  }, [engine, version]);
  ...
  const pause = () => {
    try {
      ...
      console.error("[Playback] pause failed", e);
    }
  };
  const seekMs = (pos) => {
    try {
  ...
  const setMasterVolume = (v) => {
    ...
    try {
      engine.master &&
        (engine.master.gain.gain.value = Math.max(0, Math.min(1, v / 100)));
    } catch {}
  };
